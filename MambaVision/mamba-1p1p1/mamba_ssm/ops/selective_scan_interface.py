...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                ...
                C_proj_bias=None, delta_softplus=True,  saveAttnMat=False,saveAttnVec=False, old_attention=False, compute_attn_gradients=False):
        """
             xz: (batch, dim, seqlen)
        """
        Test = False
        ...
        if Test: conv1d_bias = conv1d_bias*0 #We assume the conv bias in the attention matrix
        conv1d_out = causal_conv1d_cuda.causal_conv1d_fwd(x, conv1d_weight, conv1d_bias,None, None,None, True)
        if Test or saveAttnMat or saveAttnVec : conv1d_out_no_act = causal_conv1d_cuda.causal_conv1d_fwd(x, conv1d_weight, conv1d_bias,None,None,None, None)
        ...
        CLSposition = 98
        if Test or saveAttnMat:
            attn_mat = compute_last_attn_matrix(delta.to(torch.float32), delta_bias.to(torch.float32), A.to(torch.float32), B.to(torch.float32), C.to(torch.float32), L, x.shape, dtype=torch.float32)
            M = conv2Mat(conv1d_weight,x.shape[-1]).to(x.device).transpose(-1,-2)
            gate = torch.diag_embed(F.silu(z))
            Z = torch.diag_embed(torch.sigmoid(conv1d_out_no_act))
        ...
        # Attention Vector
        if Test or saveAttnVec:
            ablate_gate = False
            ablate_conv = False
            ablate_act = False
            attn_mat = compute_last_attn_vector(delta.to(torch.float32), delta_bias.to(torch.float32), A.to(torch.float32), B.to(torch.float32), C.to(torch.float32), L, x.shape, dtype=torch.float32, pos=CLSposition,numlines=1) # B H L 4
        ...
        if saveAttnMat:
            return out_z, attn_mat_fin
        elif compute_attn_gradients and saveAttnVec:
            return out_z, attn_vec_fin, x
        elif saveAttnVec:
            return out_z, attn_vec_fin 
        else:
            return out_z

    @staticmethod
    @custom_bwd
    def backward(ctx, dout, dummy1 = False, Dummy2=False):
        ...
        dx, dconv1d_weight, dconv1d_bias, _ = causal_conv1d_cuda.causal_conv1d_bwd(
            x, conv1d_weight, conv1d_bias, dconv1d_out, None,None,None,None,True,  False
        )
        ...
        return (dxz, dconv1d_weight, dconv1d_bias, dx_proj_weight, ddelta_proj_weight,
                dA, dB, dC, dD,
                ddelta_bias if delta_bias is not None else None,
                dB_proj_bias, dC_proj_bias, None, None, None, None, None)
...
def bimamba_inner_fn(
    ...
    out_proj_weight, out_proj_bias,
    A, A_b, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
    C_proj_bias=None, delta_softplus=True,saveAttnMat=False, saveAttnVec=False
):
    return BiMambaInnerFn.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              out_proj_weight, out_proj_bias,
                              A, A_b, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus, saveAttnMat=saveAttnMat, saveAttnVec=saveAttnVec)
...
def mamba_inner_fn_no_out_proj(
    ...
    return MambaInnerFnNoOutProj.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus, saveAttnMat, saveAttnVec, old_attention, compute_attn_gradients)
...
def compute_last_attn_matrix(delta, delta_bias, A, B, C, L, x_shape, dtype=torch.float16):
...
def compute_last_attn_vector(delta, delta_bias, A, B, C, L, x_shape, dtype=torch.float16, pos=98,numlines=4):
    dt = F.softplus(delta + delta_bias.unsqueeze(0).unsqueeze(-1))
    dA = torch.exp(torch.einsum("bdl,dn->bldn", dt, A))
    dB = torch.einsum("bdl,bnl->bldn", dt, B.squeeze(1))
    #AttnMatrixOverCLS = torch.zeros((x_shape[0], x_shape[1], x_shape[2], x_shape[2]),requires_grad=True).to(dtype).to(dA.device) #BHLL: L vectors per batch and channel
    ...
    for rr in range(numlines):
        ...
        for c in range(r+1):
            ...
            if c < r:
                # for i in range(r-c):
                #     currA = currA*dA[:,r-i,:,:]
                cumprod_result = torch.cumprod(dA[:, c+1:r+1, :, :], dim=1)
                cumprod_result = cumprod_result[:, -1, :, :]
                
                currA = cumprod_result
            currB = dB[:,c,:,:]
...
# def conv2Matsub(conv1d_weight, L):
#     num_of_lines = 5
#     cls_pos = 98
#     H, D = conv1d_weight.shape
#     conv1d_weight = torch.flip(conv1d_weight,[-1])
#     # Initialize the matrix with zeros
#     M = torch.zeros(H, num_of_lines, L).to(conv1d_weight.device)
    
#     # Fill the matrix with the kernel weights
#     for h in range(H):
#         for i in range(cls_pos,cls_pos+num_of_lines):
#             ii = i - cls_pos
#             # Set the diagonal and the next (D-1) positions, respecting the input length L
#             M[h, ii, ii:ii+D] = conv1d_weight[h, :max(0, D - (i + D - L))]
#     return M
...
def compute_middle_attn_vector_no_loops(delta, delta_bias, A, B, C, L, x_shape, cls_position=None, full_vector=False):
    ...
    if full_vector:
        ...
        curr_B = dB[:,cls_pos-1,:,:]
        AttnVecorOverCLS[:,:,cls_pos-1] = torch.sum(curr_C.squeeze(1)*curr_B, axis=-1)
