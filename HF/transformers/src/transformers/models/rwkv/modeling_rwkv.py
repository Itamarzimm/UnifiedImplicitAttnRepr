...
def compute_attention_matrix(time_decay, time_first, key, value):
    B, seq_length,H = key.size()
    attn_mat = torch.zeros(B,H,seq_length,seq_length)
    output = torch.zeros_like(key)

    time_decay = -torch.exp(time_decay)

    for row in range(seq_length):
        curr = torch.exp(key[:, row].float() + time_first)
        denom = curr
        attn_mat[:, :, row, row] = curr
        for col in range(row):
            curr = torch.exp((time_decay*(row-col-1))+ key[:, col].float())
            denom = denom + curr
            attn_mat[:, :, row, col] = curr

        attn_mat[:, :, row, :] = (attn_mat[:, :, row, :])*(1/ denom).unsqueeze(-1)
    out = torch.einsum('BHLN,BNH->BLH', attn_mat, value)
    return out, attn_mat


def compute_attention_vec(time_decay, time_first, key, value):
    B, seq_length,H = key.size()
    attn_vec = torch.zeros(B,H,seq_length)
    output = torch.zeros_like(key)

    time_decay = -torch.exp(time_decay)

    for row in range(seq_length-1,seq_length):
        curr = torch.exp(key[:, row].float() + time_first)
        denom = curr
        attn_vec[:, :, row] = curr
        for col in range(row):
        ...
        attn_vec[:, :, :] = (attn_vec[:, :, :])*(1/ denom).unsqueeze(-1)

    out = torch.einsum('BHL,BLH->BH', attn_vec, value)
    return out, attn_vec


def rwkv_linear_attention(time_decay, time_first, key, value, compute_attn_matrix = False, compute_attn_vec = False, state=None, return_state=False):
    no_cuda = any(t.device.type != "cuda" for t in [time_decay, time_first, key, value])
    # Launching the CUDA kernel for just one token will actually be slower (there is no for loop in the CPU version
    # in this case).
    one_token = key.size(1) == 1
    if rwkv_cuda_kernel is None or no_cuda or one_token or compute_attn_matrix or compute_attn_vec:
        out = rwkv_linear_attention_cpu(time_decay, time_first, key, value, state=state, return_state=return_state)
        test = False
        if test or compute_attn_vec:
            out_alt_vec, attn_vec = compute_attention_vec(time_decay, time_first, key, value)
        if test or compute_attn_matrix:
            out_alt, attn_mat = compute_attention_matrix(time_decay, time_first, key, value)
        if test :
            max_diff = (out_alt-out[0]).abs().max()
            print("diff:", max_diff)
            max_diff_cls = (out_alt_vec - out[0][:,-1,:]).abs().max()
            print("diff cls:", max_diff)
            assert max_diff < 1e-3
            assert max_diff_cls < 1e-3
            #exit()
...
class RwkvSelfAttention(nn.Module):
    ...
    def forward(self, hidden, state=None, use_cache=False):
        ...
        out = rwkv_linear_attention(
            ...
            compute_attn_vec=self.compute_attn_vec,
        )
        if self.compute_attn_vec:
            rwkv, layer_state = out[0]
            gate = (receptance.permute(0, 2, 1))[:,:,-1]
            if not self.ablate_gate:
                self.attn_vec = gate.unsqueeze(-1) * out[1]
            else:
                self.attn_vec = out[1]
        elif self.compute_attn_matrix:
            rwkv, layer_state = out[0]
            gate = torch.diag_embed(receptance.permute(0, 2, 1))
            if not self.ablate_gate:
                self.attn_matrix = gate @ out[1]
            else:
                self.attn_matrix = out[1]
            #save_attn_matrix(self.attn_matrix, self.layer_id)
        else:
            rwkv, layer_state = out
...
def save_attn_matrix(attn_matrix, layer_id):
    normalize_attn_mat = lambda attn_mat: (attn_mat - torch.min(attn_mat)) / (
                torch.max(attn_mat) - torch.min(attn_mat))
    B,H,L,_ = attn_matrix.shape
    for b in range(B):
        for h in range(H):
            curr_attn_mat = normalize_attn_mat(attn_matrix[b,h].abs())
            # savefig:
            plt.figure(figsize=(4, 4))
            # plt.imshow(curr_map, cmap='gray')
            plt.imshow(curr_attn_mat.detach().cpu().numpy(), cmap='viridis', aspect='auto', vmin=0, vmax=1)
            plt.colorbar()
            plt.axis('off')  # Turn off the axis for better visualization
            plt.savefig("C:\Projects\Papers\XAI-mamba-griffin-nips24\RWKV\images_rwkv\LayerIdx-" + str(layer_id)+ "-B-" + str(b)+"-H-" + str(h) + ".png", dpi=300)
            plt.close()