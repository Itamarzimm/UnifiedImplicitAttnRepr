...
class RecurrentGemmaRglru(nn.Module):
    ...
    def forward(
        ...
        compute_attn_matrix : bool,
        compute_attn_vec: bool,
        ...
        dtype = torch.float32
        B, L, H = activations.shape
        if compute_attn_matrix:
            attn_mat = torch.zeros((B, H, L, L)).to(dtype)
            for r in range(L):
                for c in range(r + 1):
                    curr = input_gate[:, c, :] * multiplier[:, c, :]
                    if r == c:
                        currElement = curr
                    else:
                        mult = 1
                        for s in range(c + 1, r + 1):
                            mult = mult * recurrent_gate[:, s, :]
                        currElement = mult * curr
                    attn_mat[:, :, r, c] = currElement
            #attn_mat = attn_mat.cuda()
            self.attn_mat = attn_mat
        if compute_attn_vec:
            attn_vec = torch.zeros((B, H, L)).to(dtype)
...
class RecurrentGemmaRecurrentBlock(nn.Module):
    ...
    def forward(
        ...
        B, seq_len, _ = input_states.shape
        ...
        Test = False
        x_branch = self.rg_lru(x_branch.transpose(1, 2), position_ids, self.compute_attn_matrix or Test, self.compute_attn_vec or Test)
        if self.compute_attn_matrix or Test:
            M = conv2Mat(self.conv_1d.weight.squeeze(1), seq_len).to(x_branch.device).transpose(-1, -2)
            gate = torch.diag_embed(y_branch.permute(0, 2, 1))
            if not self.ablate_gate and not self.ablate_conv:
                total_mat = gate @ (self.rg_lru.attn_mat @ M)
            elif self.ablate_gate and not  self.ablate_conv:
                total_mat = (self.rg_lru.attn_mat @ M)
            elif self.ablate_conv and not self.ablate_gate:
                total_mat = gate @ (self.rg_lru.attn_mat)
            else:
                total_mat = (self.rg_lru.attn_mat)

            self.attn_matrix = total_mat
        if self.compute_attn_vec or Test:
            M = conv2Mat(self.conv_1d.weight.squeeze(1), seq_len).to(x_branch.device).transpose(-1, -2)
            gate = y_branch.permute(0, 2, 1)
            if not self.ablate_gate and not self.ablate_conv:
                attn_vec = gate[:, :, -1].unsqueeze(-1) * ((self.rg_lru.attn_vec.unsqueeze(-2) @ M.unsqueeze(0)).squeeze(-2))
            elif self.ablate_gate and not  self.ablate_conv:
                attn_vec = ((self.rg_lru.attn_vec.unsqueeze(-2) @ M.unsqueeze(0)).squeeze(-2))
            elif self.ablate_conv and not  self.ablate_gate:
                attn_vec = gate[:, :, -1].unsqueeze(-1) * (self.rg_lru.attn_vec)
            else:
                attn_vec = self.rg_lru.attn_vec
            self.attn_vec = attn_vec

        if Test: print("diff cls attn:", (self.attn_matrix[:,:,-1,:]-self.attn_vec).abs().max())
